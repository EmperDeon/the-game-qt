...
void MWorldRender::drawAxis(){
	glLineWidth(3.0f); // устанавливаю ширину линии приближЄнно в пиксел€х
	// до вызова команды ширина равна 1 пикселю по умолчанию

	glColor4f(1.00f, 0.00f, 0.00f, 1.0f); // устанавливаетс€ цвет последующих примитивов
	// ось x красного цвета
	glBegin(GL_LINES); // построение линии
	glVertex3f( 1.0f,  0.0f,  0.0f); // перва€ точка
	glVertex3f(-1.0f,  0.0f,  0.0f); // втора€ точка
	glEnd();
...
void MWorldRender::getVertexArray(){
	...
	// начальные значени€ дл€ икосаэдра
...
void MWorldRender::getColorArray(){
	for (int i=0; i<12; i++){
		ColorArray[i][0]=0.1f*(qrand()%11); // R - красна€ составл€юща€
		ColorArray[i][1]=0.1f*(qrand()%11); // G - зелЄна€ составл€юща€
		ColorArray[i][2]=0.1f*(qrand()%11); // B - син€€ составл€юща€
